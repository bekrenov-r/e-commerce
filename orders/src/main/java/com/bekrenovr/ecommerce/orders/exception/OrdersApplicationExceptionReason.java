...
public enum OrdersApplicationExceptionReason implements EcommerceApplicationExceptionReason {
    ...
    NON_EXISTENT_ITEMS_IN_ORDER("Order request contains non-existent items", HttpStatus.NOT_FOUND),
    ...
    ITEM_ALREADY_IN_CART("Item is already added to cart", HttpStatus.CONFLICT),
    CART_ENTRY_NOT_FOUND("Cart does not contain item entry with id [%s]", HttpStatus.NOT_FOUND),
    CANNOT_CANCEL_ORDER("Cannot cancel order with status [%s]", HttpStatus.CONFLICT);
